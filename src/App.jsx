...
import { 
  ...
  MousePointer2, Search, Layout, Settings
} from 'lucide-react';
...
export default function App() {
  ...
  const allServices = [
    { id: 'iot', category: 'Infraestructura', title: "Oficinas Inteligentes y Domótica (IoT)", icon: Cpu, color: "orange", description: "Transformamos espacios de trabajo en entornos autónomos y eficientes para reducir costos.", items: ["Sensores de presencia y clima", "Iluminación automatizada", "Gestión remota de espacios"] },
    { id: 'network', category: 'Infraestructura', title: "Cableado Estructurado", icon: Network, color: "blue", description: "La base física de tu red con certificación y estándares internacionales.", items: ["Instalación profesional", "Certificación Fluke", "Auditoría y Mantenimiento"] },
    { id: 'video', category: 'Infraestructura', title: "Videovigilancia y Seguridad Física", icon: Video, color: "slate", description: "Protección integral mediante analítica de video y control físico de áreas.", items: ["CCTV IP Alta Resolución", "Control de Acceso Biométrico", "Integración de Red"] },
    { id: 'voip', category: 'Infraestructura', title: "Telefonía IP y Comunicaciones", icon: Phone, color: "teal", description: "Comunicaciones unificadas de alta fidelidad para empresas modernas.", items: ["Centrales VoIP / PBX Virtual", "Videoconferencia profesional", "Movilidad empresarial"] },
    { id: 'cyber', category: 'Operaciones', title: "Ciberseguridad", icon: ShieldCheck, color: "rose", description: "Protección avanzada de datos y blindaje contra amenazas externas.", items: ["Cumplimiento ISO 27001", "Firewall y Redes Seguras", "Phishing y Vulnerabilidades"] },
    { id: 'support', category: 'Operaciones', title: "Soporte y Mantenimiento TI", icon: Headphones, color: "emerald", description: "Garantizamos la continuidad operativa de tu infraestructura 24/7.", items: ["Contratos Recurrentes", "Mesa de Ayuda (Help Desk)", "Mantenimiento Preventivo"] },
    { id: 'data', category: 'Operaciones', title: "Gestión de Datos y Backup", icon: Database, color: "cyan", description: "Seguridad y resiliencia para la información vital de tu negocio.", items: ["Servidores NAS/SAN", "Backup en la Nube", "Recuperación ante desastres"] },
    { id: 'products', category: 'Operaciones', title: "Productos Digitales y Plantillas", icon: Package, color: "pink", description: "Herramientas listas para potenciar la productividad inmediata.", items: ["Plantillas Excel avanzadas", "Manuales PDF Inteligentes", "Packs de Recursos"] },
    { id: 'dev', category: 'Estrategia', title: "Desarrollo Web y Soluciones Low-Code", icon: Code2, color: "blue", description: "Digitalización ágil para procesos corporativos de alta velocidad.", items: ["Sitios Web Corporativos", "Apps de gestión Low-Code", "Integración vía API"] },
    { id: 'ia', category: 'Estrategia', title: "Automatización con IA", icon: Bot, color: "purple", description: "Inteligencia artificial aplicada para eliminar burocracia manual.", items: ["Chatbots de atención", "Flujos de trabajo IA", "Reportes y Predicción"] },
    { id: 'consulting', category: 'Estrategia', title: "Consultoría y Transformación Digital", icon: Briefcase, color: "amber", description: "Estrategia y acompañamiento en tu evolución tecnológica.", items: ["Diagnóstico inicial", "Hoja de ruta digital", "Asesoría en Licitaciones"] },
    { id: 'training', category: 'Estrategia', title: "Capacitación y Formación", icon: GraduationCap, color: "indigo", description: "Transferencia de conocimiento técnico para empoderar a tu equipo.", items: ["Cursos presenciales", "Academia Online", "Workshops de IA"] }
  ];
    ...
    <div className="min-h-screen bg-white font-sans text-slate-900 selection:bg-blue-100 selection:text-blue-700 flex flex-col">
            ...
            <div className="hidden md:flex items-center space-x-1">
              ...
              <div className="group relative cursor-pointer">
                ...
                <div className="absolute top-full left-1/2 -translate-x-1/2 w-[850px] bg-white shadow-2xl rounded-2xl border border-slate-100 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 p-8 grid grid-cols-3 gap-8 mt-2">
                  {['Infraestructura', 'Operaciones', 'Estrategia'].map(cat => (
                    <div key={cat} className="space-y-4 text-left text-xs">
                      <h4 className="font-black text-slate-400 uppercase tracking-widest mb-4">{cat}</h4>
      ...
      {currentPage === 'home' && (
        <main className="animate-in fade-in duration-700">
          ...
          <div className="border-y border-slate-100 bg-white py-16">
            <div className="max-w-7xl mx-auto px-4 grid grid-cols-2 md:grid-cols-4 gap-12">
              {[
                ...
                { l: "Estándar de Seguridad", v: "Ciberseguridad" },
                { l: "Experiencia Técnica", v: "+6 Años" }
              ].map((stat, i) => (
          ...
          {/* SECCIÓN "¿TE SUENA FAMILIAR?" (DOLORES DEL CLIENTE) */}
          ...
          {/* SECCIÓN SOLUCIONES 360° (PILARES) */}
...
// --- SUBPÁGINA NOSOTROS (Mantiene cambios estratégicos) ---
function AboutPage({ navigate }) {
  return (
    <div className="bg-white min-h-screen animate-in fade-in duration-700">
      <section className="bg-slate-900 text-white py-24 md:py-40 px-4 relative overflow-hidden">
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center relative z-10">
          <div className="text-left space-y-8">
            <h1 className="text-5xl md:text-7xl font-black tracking-tighter leading-none">
              Arquitectos de tu <br/>
              <span className="text-blue-500">evolución digital.</span>
            </h1>
            <p className="text-xl text-slate-400 font-medium leading-relaxed max-w-xl">
              Vimos que muchas Pymes en Chile estaban atrapadas en procesos obsoletos e infraestructuras vulnerables. Fundamos Omni TI para traer la eficiencia de la IA y el estándar corporativo al mercado local.
            </p>
            <div className="flex items-center gap-6">
              <div className="p-4 bg-white/5 border border-white/10 rounded-2xl">
                <h4 className="text-blue-400 font-black text-2xl">Propósito</h4>
                <p className="text-xs text-slate-500 font-bold uppercase tracking-widest mt-1">Transformar tecnología en ventaja.</p>
              </div>
            </div>
          </div>
          <div className="hidden lg:block relative">
            <div className="w-full aspect-square bg-blue-600/10 rounded-[4rem] border-2 border-white/5 flex items-center justify-center relative overflow-hidden group">
               <Bot className="w-48 h-48 text-blue-500 opacity-20 group-hover:scale-110 transition-transform duration-1000" />
               <Sparkles className="absolute top-10 right-10 text-blue-400 animate-pulse" />
            </div>
          </div>
        </div>
        <div className="absolute top-0 right-0 w-96 h-96 bg-blue-600 rounded-full blur-[150px] opacity-10"></div>
      ...
      <section className="py-24 px-4 bg-white">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-12">
          <div className="bg-slate-50 p-12 rounded-[3.5rem] border border-slate-100 space-y-6">
            <div className="w-16 h-16 bg-white text-blue-600 rounded-2xl flex items-center justify-center shadow-sm"><UsersIcon className="w-8 h-8"/></div>
            <h3 className="text-3xl font-black text-slate-900 tracking-tight">De Técnicos a Socios</h3>
            <p className="text-slate-500 font-medium leading-relaxed">
              Aunque somos una empresa joven, nuestro equipo suma **más de 10 años de experiencia** liderando departamentos de TI y administrando infraestructuras críticas. No somos solo un proveedor; somos tu brazo técnico de confianza en Santiago.
            </p>
          </div>
          <div className="bg-blue-600 p-12 rounded-[3.5rem] text-white space-y-6 relative overflow-hidden">
            <div className="w-16 h-16 bg-white/10 text-white rounded-2xl flex items-center justify-center backdrop-blur-md"><ShieldCheck className="w-8 h-8"/></div>
            <h3 className="text-3xl font-black tracking-tight">Estándar ISO 27001:2022</h3>
            <p className="text-blue-100 font-medium leading-relaxed">
              No instalamos cables; construimos activos digitales protegidos. Cada proyecto en Omni TI se rige bajo la norma **ISO 27001**, asegurando que tu infraestructura cumpla con los más altos estándares de seguridad de la información.
            </p>
      ...
      <section className="py-24 bg-slate-50 px-4">
        <div className="max-w-7xl mx-auto text-center mb-16">
          <h2 className="text-4xl md:text-6xl font-black text-slate-900 tracking-tighter mb-4">El Camino Omni</h2>
          <p className="text-slate-500 font-medium">Nuestra metodología de 4 pasos diseñada para evitar el caos tecnológico.</p>
        </div>
        <div className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {[
            { n: "01", t: "Auditoría", d: "Analizamos tu desorden actual y riesgos de seguridad.", i: Search },
            { n: "02", t: "Diseño", d: "Creamos un plano tecnológico a la medida de tu escala.", i: Layout },
            { n: "03", t: "Ejecución", d: "Implementamos sin interrumpir tu flujo de negocio.", i: Rocket },
            { n: "04", t: "Soporte", d: "Nunca te dejamos solo; monitoreamos 24/7.", i: Headphones }
          ].map((step, i) => (
            <div key={i} className="bg-white p-10 rounded-[3rem] border border-slate-200 shadow-sm relative group">
              <span className="text-6xl font-black text-slate-50 absolute top-4 right-8 group-hover:text-blue-50 transition-colors">{step.n}</span>
              <step.i className="w-10 h-10 text-blue-600 mb-6 relative z-10" />
              <h4 className="text-2xl font-black text-slate-900 mb-4 relative z-10">{step.t}</h4>
              <p className="text-slate-500 text-sm font-medium leading-relaxed relative z-10">{step.d}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="py-24 px-4">
        <div className="max-w-5xl mx-auto bg-slate-900 rounded-[4rem] p-12 md:p-20 text-center text-white relative overflow-hidden">
          <div className="relative z-10 space-y-8">
            <div className="inline-flex items-center gap-3 px-6 py-2 bg-white/5 border border-white/10 rounded-full text-blue-400 font-black text-xs uppercase tracking-widest">
              <MapPin className="w-4 h-4" /> Basados en Santiago de Chile
            </div>
            <h2 className="text-4xl md:text-6xl font-black tracking-tighter leading-none">¿Necesitas un socio local <br/> que hable tu idioma?</h2>
            <p className="text-slate-400 text-xl font-medium max-w-xl mx-auto leading-relaxed">
              Entendemos el mercado chileno y la urgencia de soporte en terreno. Estamos a un café o una llamada de distancia.
            </p>
            <button 
              onClick={() => navigate('contact')}
              className="bg-blue-600 text-white px-12 py-6 rounded-3xl font-black text-xl hover:bg-blue-700 transition-all shadow-xl active:scale-95"
            >
              Agendar Visita Técnica
            </button>
          </div>
          <Bot className="absolute -bottom-20 -left-20 w-80 h-80 text-white/5 -rotate-12" />
        </div>
      </section>
    </div>
  );
...
function AssessmentPage({ navigate }) { 
  ...
  const services = [ { id: 'iot', name: 'Oficinas Smart (IoT)', icon: Cpu, desc: 'Espacios inteligentes y control eficiente.' }, { id: 'dev', name: 'Desarrollo & Low-Code', icon: Code2, desc: 'Apps y webs a medida de alta velocidad.' }, { id: 'network', name: 'Cableado Estructurado', icon: Network, desc: 'Redes físicas certificadas y estables.' }, { id: 'cyber', name: 'Ciberseguridad', icon: ShieldCheck, desc: 'Blindaje total contra amenazas digitales.' }, { id: 'ia', name: 'Automatización IA', icon: Bot, desc: 'Automatización inteligente de procesos.' }, { id: 'support', name: 'Soporte TI', icon: Headphones, desc: 'Asistencia técnica experta 24/7.' }, { id: 'video', name: 'Videovigilancia', icon: Video, desc: 'Vigilancia IP y control de acceso.' }, { id: 'consulting', name: 'Consultoría Digital', icon: Briefcase, desc: 'Estrategia para tu evolución digital.' }, { id: 'training', name: 'Capacitación TI', icon: GraduationCap, desc: 'Formación técnica para tu equipo.' }, { id: 'data', name: 'Gestión de Datos', icon: Database, desc: 'Backup seguro y recuperación ante fallos.' }, { id: 'voip', name: 'Telefonía IP', icon: Phone, desc: 'Comunicación moderna y económica.' }, { id: 'products', name: 'Productos Digitales', icon: Package, desc: 'Plantillas y herramientas digitales.' } ]; 
  const handleScore = (id, val) => setScores(prev => ({ ...prev, [id]: val })); 
  const calculateResult = () => { const total = Object.values(scores).reduce((a, b) => a + b, 0); const avg = total / services.length; if (avg < 1.7) return { level: 'Nivel Inicial', color: 'text-rose-600', bg: 'bg-rose-50', desc: 'Tu empresa depende de procesos manuales y corre riesgos de seguridad críticos.' }; if (avg < 2.5) return { level: 'Nivel en Desarrollo', color: 'text-amber-600', bg: 'bg-amber-50', desc: 'Falta integración y automatización para escalar eficientemente.' }; return { level: 'Nivel Avanzado', color: 'text-emerald-600', bg: 'bg-emerald-50', desc: 'Lideras con tecnología, el enfoque ahora debe ser la excelencia continua.' }; }; 
